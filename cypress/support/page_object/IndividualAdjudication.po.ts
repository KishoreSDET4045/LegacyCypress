...
export default class IndividualAdjudication {
    ...
    useDropDown() {
        cy.get('#profileType-input > div > select').select('Regular-test')
        cy.get('#find-profiles-button').click({force:true})
        cy.wait(3000)
        cy.get('a#person-tab').click({force:true}).contains('Person ')
        cy.wait(2000);
    }
    SelectProfile() {
        cy.get('#person-tab-active > ddiq-person-search > table > tr:nth-child(2) > td:nth-child(2) > span > a').click({force:true})
        cy.wait(5000) 
    }
    SelectNavForConfirm() {
    ...
    AddComment() {
        cy.get('div.comment-box > textarea').type('Done through automastion')
        cy.contains('button', 'Adjudicate').click({force:true})
        cy.wait(5000)  
    }
    SelectNavForRemove() {
